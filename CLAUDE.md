...
- ccfocus-app (Swift/SwiftUI): メニューバー常駐。FSEventsでログを追尾し、状態復元/UI表示/AppleScript focusを担う
...
xcodegen generate --spec ccfocus-app/project.yml --project ccfocus-app/  # xcodeproj生成
xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -configuration Debug build  # Swiftビルド
xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-appTests -configuration Debug test  # Swiftテスト
```

## Development Rules

- PRを作成してmainにマージする。mainへの直接pushは避ける
- ccfocus-app/ccfocus-app.xcodeprojはproject.ymlから生成されるため、gitに含めない。Swiftファイルを追加した場合は`xcodegen generate`を再実行する
...
ccfocus-appのUI変更を検証する際:

```bash
pkill -x ccfocus-app 2>/dev/null
xcodegen generate --spec ccfocus-app/project.yml --project ccfocus-app/
xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -configuration Debug build 2>&1 | tail -5
open ~/Library/Developer/Xcode/DerivedData/ccfocus-app-*/Build/Products/Debug/ccfocus-app.app
```
