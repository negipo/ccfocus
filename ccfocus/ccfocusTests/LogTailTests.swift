...
final class LogTailTests: XCTestCase {
    func testReadNewReturnsAllLinesFirstTime() throws {
        ...
        let fileURL = dir.appendingPathComponent("2026-04-16.jsonl")
    ...
    func testReadNewReturnsOnlyAppendedLines() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(UUID().uuidString)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let fileURL = dir.appendingPathComponent("d.jsonl")
        try "line1\n".write(to: fileURL, atomically: true, encoding: .utf8)
        let reader = LogTail.Reader()
        _ = reader.readNew(url: fileURL)
    ...
    func testReadNewHandlesPartialLineByReadingAgainNextTime() throws {
        ...
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let fileURL = dir.appendingPathComponent("d.jsonl")
        try "lineA\npart".write(to: fileURL, atomically: true, encoding: .utf8)
        let reader = LogTail.Reader()
        let first = reader.readNew(url: fileURL)
        XCTAssertEqual(first, ["lineA"])
        let handle = try FileHandle(forWritingTo: fileURL)
        try handle.seekToEnd()
        try handle.write(contentsOf: Data("ial\n".utf8))
        try handle.close()
        let second = reader.readNew(url: fileURL)
