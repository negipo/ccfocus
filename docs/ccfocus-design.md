...
複数のClaude Code セッションが走るGhostty paneを横断的に追跡し、通知が来たpaneへ瞬時に飛べるようにする macOSメニューバー常駐アプリ。状態の真実の源は append-only イベントログ (jsonl)、ccfocus.app は表示・操作層。

## 登場要素

- User: 人間
- Ghostty: ターミナルアプリ本体 (1.3+ AppleScript対応)
- Shell: zsh/bash/fish。Ghostty Surface内で動く (ccfocusは触らない)
...
- ccfocus-logger (CLI): 軽量別binary (Rust想定)。Hookから背景で呼ばれ、AppleScriptで pane を特定し、イベントログにJSON1行をappendしてすぐ終了
- イベントログ: 日次ローテーションされるappend-only jsonl ファイル群。永続的な真実の源 (source of truth)
- ccfocus.app: メニューバー常駐するMac Nativeアプリ本体 (1プロセス、SwiftUI + Cocoa)。起動時にイベントログを再生して状態復元、その後ファイル監視でリアルタイム追従、UI更新と AppleScript発行を担う
- AppleScript: GhosttyのScripting Bridgeインターフェース

## ccfocus.app の起動性質

ccfocus.app はソケットの listener ではない。状態の真実の源はイベントログファイルであり、ccfocus.app はその表示・操作層に過ぎない。

- ccfocus.app未起動でもCLIは普通にログ追記する。Claude Codeの起動は完全に独立
- ccfocus.appを後発で起動した場合、当日のログを先頭から再生して現状を復元
- ユーザにLogin Item登録を推奨はするが必須ではない
- ccfocus.appがクラッシュしてもログは無事。再起動で元通り
...
```
~/Library/Application Support/ccfocus/events/YYYY-MM-DD.jsonl
```
...
```json
{"ts":"2026-04-16T09:12:34.567Z","event":"session_start","session_id":"abc","terminal_id":"B9BE...","cwd":"/Users/negipo/src/github.com/negipo/ccfocus","git_branch":"po/ccfocus-design","claude_pid":12345,"claude_start_time":"Wed Apr 16 09:12:34 2026","claude_comm":"claude"}
...
保持期間は7日。8日以上前の jsonl ファイルは ccfocus.app 起動時 および 1日1回の周期タスクで削除。

ccfocus.app 起動時の SessionRegistry 再構築は events/ 内の全 jsonl ファイル (最大7個) を古い順に走査して行う。これにより:

- 日跨ぎ・週跨ぎセッション (claude --resume で何日も続いた作業) も SessionRegistry に正しく復元される
- 走査時間は数十ms以下、起動体感に影響しない

トレードオフ: 8日以上継続するセッションは ccfocus.app の再起動後に履歴が失われる。レアケースとして許容。気になる場合は保持期間を伸ばすconfigで対応する。
...
```mermaid
flowchart TD
  A[ccfocus-logger session-start 起動] --> B[AppleScript で全 terminal 列挙]
...
sequenceDiagram
  ...
  participant CLI as ccfocus-logger (background)
  participant AS as Ghostty AppleScript
  participant Log as イベントログ jsonl
  participant App as ccfocus.app (起動中なら)
  ...
  Hook->>CLI: ccfocus-logger session-start (session_id, cwd, claude_pid, claude_start_time, claude_comm) &!
...
- ccfocus.app が未起動でも CLI は完全に動く。後で起動すればログ再生で復元
...
sequenceDiagram
  ...
  participant CLI as ccfocus-logger (background)
  participant Log as イベントログ jsonl
  participant App as ccfocus.app
  ...
  Hook->>CLI: ccfocus-logger <event> (session_id, message) &!
...
将来的に走査が重くなったら snapshot 最適化を入れる ([negipo/ccfocus#3](https://github.com/negipo/ccfocus/issues/3))。
...
- stale は時間ベースの自動遷移 (ccfocus.app 内部タイマーで判定、ログには書かない)
...
両方を ccfocus.app 内の10秒周期タスクで検出する。

### PIDベース3点チェック (一次判定)

PID単独では再利用問題で誤判定する (ccfocus.app停止中にClaude終了 → 同PIDが別プロセスに付与 → 再起動時の `kill()` が誤って成功)。これを防ぐため、session_start時に取得した `(claude_pid, claude_start_time, claude_comm)` の3点全一致を要求する。
...
判定はin-memoryのみ。ログには書かない。ccfocus.app 再起動時はまた生存確認から再構築する。
...
## 内部データモデル (ccfocus.app 内、イベントログから派生する派生状態)
...
terminal_id が null のまま記録されたセッションは「未紐付け」バッジを付けて表示。クリックで「該当paneを選んでください」UIを開き、現在開いているpane一覧から手動選択。選択結果は ccfocus.app 内の補助マップに保存し、以降は紐付け済みとして扱う。
...
{
  "hooks": {
    "SessionStart":     [{"hooks": [{"type": "command", "command": "ccfocus-logger session-start"}]}],
    "Notification":     [{"hooks": [{"type": "command", "command": "ccfocus-logger notification"}]}],
    "Stop":             [{"hooks": [{"type": "command", "command": "ccfocus-logger stop"}]}],
    "PreToolUse":       [{"hooks": [{"type": "command", "command": "ccfocus-logger pre-tool-use"}]}],
    "UserPromptSubmit": [{"hooks": [{"type": "command", "command": "ccfocus-logger user-prompt-submit"}]}]
  }
}
```
...
将来は ccfocus を Claude plugin として配布し、`/plugins install ccfocus` 一発で hook 設定まで完了させたい。
...
`alias claude=ccfocus-wrap claude` で claude起動を wrapper でくるみ、起動時に Ghostty AppleScript で focused terminal の id を環境変数に注入してから exec する。ペイン特定は100%確実。
...
Hook内で `printf '\033]0;CCFOCUS-MARKER-<uuid>\007'` でタイトルに一意マーカーを書き、ccfocus.app側で AppleScript列挙してマーカーを持つterminalを発見、id を取得して紐付ける。
...
`.zshrc` から `ccfocus register --async ... &!` でバックグラウンド送信、osascript 呼び出しは背景プロセスで実行。socket 経由で常駐アプリへ通知。
...
### γ: ccfocus.app 経由 split + 環境変数注入 方式

ccfocus.app から AppleScript の `split` コマンドで新terminalを作成、その時に `surface configuration` の `environment variables` に `CCFOCUS_PANE_UUID=<uuid>` を渡す。AppleScript が返す terminal.id と uuid を内部マップに記録、Hookでは env から uuid を読むだけ。race完全消滅。

却下理由: 「claude用split は ccfocus.app 経由で開く」という制約をユーザワークフローに強いる。ccfocus を介さず開いた pane で claude を起動する既存ワークフローが捨てられる (未紐付け扱いになる)。コア体験に強い縛りが入りすぎる。
...
- ccfocus-logger session-end が即座にイベントログに記録
...
- ccfocus-logger は単に「surface_id を terminal.id に変換してログ追記」する軽量呼び出しに置き換わる
...
- ccfocus.app の起動状態に CLI は依存しない (常にログにappendするだけ)
...
- Codex CLI / Aider / opencode などへの対応が必要になったら、対応Hook相当の口を別途用意する。同じ `ccfocus-logger` を呼べばイベントログには同じ形式で記録され、ccfocus.app側のロジックは流用できる
- 同じpaneで複数セッションが順次走る場合、SessionRegistry に過去履歴をぶら下げる拡張で対応可能
- ログが大きくなって再生が重くなったら snapshot 最適化 ([negipo/ccfocus#3](https://github.com/negipo/ccfocus/issues/3))
- branch / cwd / message による検索・絞り込み ([negipo/ccfocus#1](https://github.com/negipo/ccfocus/issues/1))
- `claude --resume <session_id>` コマンドの表示とクリップボードコピー ([negipo/ccfocus#2](https://github.com/negipo/ccfocus/issues/2))
