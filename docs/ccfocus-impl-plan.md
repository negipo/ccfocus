...
Architecture: Claude Code Hookから呼ばれるRust製CLI `ccfocus-logger` がAppleScriptでGhostty paneを特定してjsonlに書き込み、SwiftUIの `ccfocus.app` がFSEventsでログを追尾して状態を復元・UI表示・AppleScript focusを行う。状態の真実の源はファイル、appはその投影層。

Tech Stack:
- ccfocus-logger: Rust (edition 2021)、serde_json、clap、std::process::Command (AppleScript / ps / git実行)
- ccfocus.app: Swift 5.9+、SwiftUI、MenuBarExtra (macOS 13+)、CoreServices (FSEvents) + 追記ポーリング二段構え、NSAppleScript、macOS .app bundle (LSUIElement)
- プロジェクト構成: Cargo workspace (Rust CLI) + Xcode project (macOSアプリ、.app bundle生成) の monorepo
- 参照: docs/ccfocus-design.md (全設計と不変条件)
...
│       ├── log_path.rs                         # ~/Library/Application Support/ccfocus/events/YYYY-MM-DD.jsonl 解決
...
│       └── log_path.rs
├── ccfocus-app/
│   ├── ccfocus-app.xcodeproj/                  # Xcodeプロジェクト (.app ターゲット)
│   ├── ccfocus-app/
│   │   ├── Info.plist                          # LSUIElement=true、bundle identifier 等
│   │   ├── ccfocus_app.entitlements            # App sandbox無効 (AppleScript使用のため)
│   │   ├── ccfocusApp.swift                    # @main entry (MenuBarExtra)
...
コミットは `git-committing` スキルに従って作成する。メッセージ例: `chore: add ccfocus runtime verification scripts`

### Task 0.2: 検証A - SessionStart Hookの $PPID と親プロセス系列

Files:
- Modify: `~/.claude/settings.json` (一時追加 → 検証後削除)
- Output: `~/.ccfocus-verify/hook_*.txt`

- [x] Step 1: 検証用のhook設定を `~/.claude/settings.json` に一時追加

追加内容 (SessionStart array の先頭に):
```json
{"hooks": [{"type": "command", "command": "/Users/negipo/src/github.com/negipo/ccfocus/verify/verify_hook.sh"}]}
```

既存のSessionStart hookがある場合は末尾に追加する。ユーザに対し `update-config` スキル相当の編集を行う前に、現在のsettings.jsonの内容を提示して確認する。

- [x] Step 2: Ghosttyで新しいpaneを開き、そのpane内で `cd /Users/negipo/src/github.com/negipo/ccfocus && claude` を実行

Expected: Claude Codeが起動し、`~/.ccfocus-verify/hook_<timestamp>.txt` が生成される

- [x] Step 3: 生成されたtxtを確認

Run: `ls -t ~/.ccfocus-verify/ | head -1` → `hook_<timestamp>.txt`
Run: Read tool で `~/.ccfocus-verify/hook_<timestamp>.txt` を読む
...
Files:
- Output: `~/.ccfocus-verify/hook_*.txt` (Task 0.2のoutputを流用)
...
検証B完了後、検証用 verify_hook.sh と verify_ghostty.applescript はリポジトリに残すが、`~/.ccfocus-verify/` の出力と settings.json の一時hookは片付ける。
...
通知からpaneへ飛ぶ操作が成立するかを最初に確認する。成立しなければ以降のUI価値がまるごと消えるため、ccfocus.app実装前に判断する。
...
ccfocus.appが実行するコンテキスト (バックグラウンド) に近づけるため、`caffeinate -s osascript ... &` のようにバックグラウンド実行で試すバリエーションも試す。
...
新規ファイル `Cargo.toml` (workspace)、`ccfocus-logger/Cargo.toml`、`ccfocus-app/ccfocus-app.xcodeproj` を作成する。ユーザ指示で最初の足場作りは確認とされているため、このフェーズの開始前にユーザに一声かける。
...
### Task 1.1: Cargo workspace と ccfocus-logger スケルトン

Files:
- Create: `Cargo.toml`
- Create: `ccfocus-logger/Cargo.toml`
- Create: `ccfocus-logger/src/main.rs`
- Create: `ccfocus-logger/src/cli.rs`
- Create: `.gitignore`
...
- [x] Step 3: `ccfocus-logger/Cargo.toml` を作成
...
- [x] Step 4: `ccfocus-logger/src/main.rs` を作成
...
- [x] Step 5: `ccfocus-logger/src/cli.rs` を作成
...
Run: `cargo build -p ccfocus-logger`
Expected: warning OK、error なし

Run: `cargo run -p ccfocus-logger -- --help`
Expected: サブコマンド一覧 (session-start / notification / stop / pre-tool-use / user-prompt-submit) が表示される

- [x] Step 7: コミット

```bash
git add Cargo.toml ccfocus-logger/ .gitignore
git commit
```

メッセージ例: `chore: bootstrap ccfocus-logger Rust workspace and CLI skeleton`

### Task 1.2: ccfocus-app Xcode project スケルトン (ユーザ手動操作が必要)

Files:
- Create: `ccfocus-app/ccfocus-app.xcodeproj/` (ユーザがXcode GUIで作成)
- Create: `ccfocus-app/ccfocus-app/ccfocusApp.swift`
- Create: `ccfocus-app/ccfocus-app/Info.plist` (Xcode自動生成、後でLSUIElementを追加)
- Create: `ccfocus-app/ccfocus-app/ccfocus_app.entitlements`
- Create: `ccfocus-app/ccfocus-appTests/SmokeTests.swift`
...
- Template: macOS → App
- Product Name: `ccfocus-app`
- Team: (任意、個人開発なら空でもビルドは通る)
- Organization Identifier: `com.negipo`
- Bundle Identifier: `com.negipo.ccfocus-app`
- Interface: SwiftUI
- Language: Swift
- Include Tests: チェック
- 保存先: `/Users/negipo/src/github.com/negipo/ccfocus/ccfocus-app/`
- 作成後に `ccfocus-app/ccfocus-app.xcodeproj` があることを確認

Xcode上での追加設定:
- Target `ccfocus-app` → Signing & Capabilities → "Automatically manage signing" はON (開発中は自分のApple ID)
...
完了確認:
Run: `ls ccfocus-app/ccfocus-app.xcodeproj/project.pbxproj`
Expected: 存在する

- [x] Step 2: 生成された `ccfocus-app/ccfocus-app/ccfocus_appApp.swift` (Xcode自動命名) を `ccfocusApp.swift` にリネームし、中身を差し替える
...
struct CcfocusApp: App {
    var body: some Scene {
        MenuBarExtra("ccfocus", systemImage: "bubble.left.and.bubble.right") {
            Text("ccfocus (skeleton)")
            Divider()
...
Xcodeが自動生成した `ccfocus_appTests.swift` を `SmokeTests.swift` にリネームし、中身を差し替え:
...
注: `@testable import ccfocus_app` はProduct Moduleのimport。Xcode のTargetにTest対象を追加して初めて通る。

- [x] Step 4: CLIビルド

Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -configuration Debug build`
Expected: `** BUILD SUCCEEDED **`

Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -configuration Debug test`
...
Run (ビルド成果物のパス):
```bash
APP=$(xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -showBuildSettings -configuration Debug | awk -F' = ' '/ BUILT_PRODUCTS_DIR /{print $2}')
open "$APP/ccfocus-app.app"
```
Expected: メニューバーにbubbleアイコンが出現、Dockアイコンは出ない (LSUIElement効いている)、クリックで "ccfocus (skeleton)" と Quit が見える
...
### Task 2.0: self-detach 機構 (ccfocus-logger 共通)
...
方針: ccfocus-logger は起動直後に「stdin payload を吸い出し → 同じ実行ファイルを `CCFOCUS_LOGGER_DETACHED=1` env 付き + 同引数で spawn → stdin に payload をpipe → 親は即 exit」する。子プロセスは env の検出で detach済みと判定し、本処理に進む。これにより hook 同期パスは数ms (プロセスspawn + payload pipe書き込み) で完了する。

Files:
- Create: `ccfocus-logger/src/detach.rs`
- Modify: `ccfocus-logger/src/main.rs` (最初の関数呼び出しとしてdetachを挟む)
- Modify: `ccfocus-logger/src/lib.rs`
- Create: `ccfocus-logger/tests/detach.rs`

- [x] Step 1: テスト (detach検出ロジックの純部分)

```rust
use ccfocus_logger::detach::{is_detached_child, child_env_marker};
...
- [x] Step 2: `ccfocus-logger/src/detach.rs` を作成
...
fn install_does_not_detach() {
    let cli = Cli::try_parse_from(["ccfocus-logger", "install"]).unwrap();
...
Run: `cargo test -p ccfocus-logger --test detach`
Expected: 2 passed

- [x] Step 5: 実機計測

settings.jsonの検証用hookに `/path/to/ccfocus-logger session-start` を仮登録し、以下を計測:

Run: `time (echo '{"session_id":"t","cwd":"/tmp"}' | ccfocus-logger session-start)`
Expected: `real` が 50ms 未満 (目標は10〜30ms)

子プロセス側の処理は別途background で走っているので、`\cat ~/Library/Application\ Support/ccfocus/events/$(date +%Y-%m-%d).jsonl` で数秒以内にsession_start行が追加されていることを確認。
...
Files:
- Create: `ccfocus-logger/src/event.rs`
- Create: `ccfocus-logger/tests/event_serde.rs`
- Modify: `ccfocus-logger/src/main.rs` (`mod event;` 追加)

- [x] Step 1: テスト `ccfocus-logger/tests/event_serde.rs` を作成
...
Run: `cargo test -p ccfocus-logger --test event_serde`
Expected: コンパイルエラー (event module 未定義)

- [x] Step 3: `ccfocus-logger/src/event.rs` を作成
...
`ccfocus-logger/src/main.rs` と同階層に `ccfocus-logger/src/lib.rs` を作成し、integration testから参照可能にする:

```rust
// ccfocus-logger/src/lib.rs
pub mod event;
```

`ccfocus-logger/Cargo.toml` の `[package]` に以下を追加:
```toml
[lib]
name = "ccfocus_logger"
path = "src/lib.rs"
...
`main.rs` は `use ccfocus_logger::...` に切り替える。
...
Files:
- Create: `ccfocus-logger/src/log_path.rs`
- Create: `ccfocus-logger/tests/log_path.rs`
- Modify: `ccfocus-logger/src/lib.rs`
...
fn events_dir_is_application_support_ccfocus_events() {
    ...
    assert_eq!(dir, std::path::PathBuf::from(format!("{}/Library/Application Support/ccfocus/events", home)));
}
...
Run: `cargo test -p ccfocus-logger --test log_path`
Expected: コンパイルエラー

- [x] Step 3: `ccfocus-logger/src/log_path.rs` を作成
...
- [x] Step 4: `ccfocus-logger/src/lib.rs` に `pub mod log_path;` 追加
...
Files:
- Create: `ccfocus-logger/src/log_writer.rs`
- Create: `ccfocus-logger/tests/log_writer.rs`
- Modify: `ccfocus-logger/src/lib.rs`
...
```rust
use ccfocus_logger::event::{Event, EventKind};
use ccfocus_logger::log_writer::append_event_to;
...
Run: `cargo test -p ccfocus-logger --test log_writer`
Expected: コンパイルエラー

- [x] Step 3: `ccfocus-logger/src/log_writer.rs` を作成
...
- [x] Step 4: `ccfocus-logger/src/lib.rs` に `pub mod log_writer;` 追加
...
Files:
- Create: `ccfocus-logger/src/timestamp.rs`
- Modify: `ccfocus-logger/src/lib.rs`

- [x] Step 1: テスト (ドキュメントテストでOK、外形ルールのみ)

```rust
// ccfocus-logger/src/timestamp.rs 内に #[cfg(test)] モジュールを置く
#[cfg(test)]
mod tests {
...
- [x] Step 2: `ccfocus-logger/src/timestamp.rs` 作成
...
Run: `cargo test -p ccfocus-logger`
...
Files:
- Create: `ccfocus-logger/src/git.rs`
- Modify: `ccfocus-logger/src/lib.rs`
...
- [x] Step 2: `ccfocus-logger/src/git.rs` 作成
...
Files:
- Create: `ccfocus-logger/src/claude_proc.rs`
- Modify: `ccfocus-logger/src/lib.rs`

注:
- ccfocus-loggerのdetach機構が入った時点で親は launchd / 子shell。よって `current_ppid()` や `getppid()` は Claude を指さない。設計 & 検証A の想定 (hook script 内で ps -p $PPID を取得) は hook script で取るもの。ccfocus-logger起動時には既に detach済みで親が変わっている。claude_pid 系は logger がforkする前 (hook内 shell) で抽出する必要がある
- 修正: Claude Code hook は `command` を直接spawnするため、ccfocus-loggerが最初に起動される時点では `$PPID = claude` (もしくはnode/bun等) である。Task 2.0 の self-detach は「current_exe を再spawn」するので、親プロセスの `$PPID` と `getppid()` の値は子プロセス内では親 (初代logger) を指す。したがって初代logger が claude_pid を確保してから子に env 渡しする形で継承する

claude_pid 取得戦略:
- 初代logger 起動時 (is_detached_child() == false) で `ps -p <getppid()> -o pid=,lstart=,comm=,command=` を実行
- 結果から CLAUDE_COMM_PATTERNS に含まれるcomm/commandかを判定、該当したら env `CCFOCUS_CLAUDE_PID=<pid>`, `CCFOCUS_CLAUDE_START=<lstart>`, `CCFOCUS_CLAUDE_COMM=<comm>` を子にも渡す
...
- [x] Step 2: `ccfocus-logger/src/claude_proc.rs` 実装
...
子側 (session_start command 内) では `std::env::var("CCFOCUS_CLAUDE_PID")` を読む。
...
Files:
- Create: `ccfocus-logger/src/ghostty.rs`
- Modify: `ccfocus-logger/src/lib.rs`
...
- [x] Step 2: `ccfocus-logger/src/ghostty.rs` 作成
...
Files:
- Create: `ccfocus-logger/src/commands/mod.rs`
- Create: `ccfocus-logger/src/commands/session_start.rs`
- Modify: `ccfocus-logger/src/cli.rs`
- Modify: `ccfocus-logger/src/lib.rs`
...
```rust
// ccfocus-logger/src/commands/session_start.rs 内
#[cfg(test)]
mod tests {
...
`ccfocus-logger/src/commands/mod.rs`:
```rust
...
`ccfocus-logger/src/commands/session_start.rs`:
```rust
...
pub fn run() -> Result<()> {
    ...
    let claude_pid = std::env::var("CCFOCUS_CLAUDE_PID").ok().and_then(|s| s.parse().ok());
...
`~/.claude/settings.json` に `ccfocus-logger session-start` を `SessionStart` に仮登録し、新しいpaneで `claude` を実行。

`~/Library/Application Support/ccfocus/events/<今日>.jsonl` に session_start 行が1行入っていることを確認:

Run: `\cat ~/Library/Application\ Support/ccfocus/events/$(date +%Y-%m-%d).jsonl`
Expected: `{"ts":"...","event":"session_start","session_id":"...","terminal_id":"...","cwd":"...","git_branch":"po/ccfocus-impl-plan","claude_pid":...,"claude_start_time":"...","claude_comm":"claude"}`
...
Files:
- Create: `ccfocus-logger/src/commands/notification.rs`
- Create: `ccfocus-logger/src/commands/stop.rs`
- Create: `ccfocus-logger/src/commands/pre_tool_use.rs`
- Create: `ccfocus-logger/src/commands/user_prompt_submit.rs`
- Modify: `ccfocus-logger/src/cli.rs`
...
Run: `\cat ~/Library/Application\ Support/ccfocus/events/$(date +%Y-%m-%d).jsonl | \cat`
...
Files:
- Create: `ccfocus-app/ccfocus-app/Event.swift`
- Create: `ccfocus-app/ccfocus-app/EventLogReader.swift`
- Create: `ccfocus-app/ccfocus-appTests/EventLogReaderTests.swift`
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/EventLogReaderTests test`
...
Files:
- Create: `ccfocus-app/ccfocus-app/SessionStatus.swift`
- Create: `ccfocus-app/ccfocus-appTests/SessionStatusTests.swift`
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/SessionStatusTests test`
Expected: 7 passed
...
Files:
- Create: `ccfocus-app/ccfocus-app/SessionRegistry.swift`
- Create: `ccfocus-app/ccfocus-appTests/SessionRegistryTests.swift`
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/SessionRegistryTests test`
Expected: 3 passed
...
Files:
- Create: `ccfocus-app/ccfocus-app/GhosttyFocus.swift`
...
enum GhosttyFocus {
    static func focus(terminalId: String) {
        ...
        if let script = NSAppleScript(source: source) {
            ...
            if let e = error {
                NSLog("[ccfocus] GhosttyFocus error: \(e)")
            }
        }
    }
}
...
Files:
- Create: `ccfocus-app/ccfocus-app/LogTail.swift`
- Create: `ccfocus-app/ccfocus-appTests/LogTailTests.swift`
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/LogTailTests test`
Expected: 3 tests passed

- [x] Step 4: 手動確認

一時的にccfocus-app内にWatcherをinitして、別paneで `\echo '{}' >> ~/Library/Application\ Support/ccfocus/events/manual.jsonl` を叩き、NSLogに通知が届くことを確認。
...
Files:
- Modify: `ccfocus-app/ccfocus-app/ccfocusApp.swift`
- Create: `ccfocus-app/ccfocus-app/MenuBarView.swift`
- Create: `ccfocus-app/ccfocus-app/AppState.swift`
...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Divider()
            Button("Quit ccfocus") { NSApp.terminate(nil) }.keyboardShortcut("q")
        }
        .padding(8)
        .frame(minWidth: 320)
    }
...
1. アプリをビルド&起動: `open $(xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -showBuildSettings -configuration Debug | awk -F' = ' '/ BUILT_PRODUCTS_DIR /{print $2}')/ccfocus-app.app`
2. settings.jsonにccfocus-loggerの5hook登録 (フェーズ2で用意したhooks設定を実行パス経由で)
...
Files:
- Create: `ccfocus-app/ccfocus-app/LivenessChecker.swift`
- Modify: `ccfocus-app/ccfocus-app/SessionRegistry.swift` (deceased遷移の口)
- Create: `ccfocus-app/ccfocus-appTests/LivenessCheckerTests.swift`
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/LivenessCheckerTests test`
Expected: 3 passed
...
Files:
- Modify: `ccfocus-app/ccfocus-app/MenuBarView.swift`
- Modify: `ccfocus-app/ccfocus-app/AppState.swift`
...
codexレビュー指摘: 同一cwd複数pane時は `terminal_id=null` となり後続イベントも再同定情報を持たないため、手動紐付け結果を永続化して ccfocus.app 再起動後にも残す経路を明記する。永続化先は `~/Library/Application Support/ccfocus/manual_pairings.json` で、キー=session_id、値=terminal_id。paneが閉じて terminal が存在しなくなった場合、生存確認周期で対応エントリも削除する。

Files:
- Create: `ccfocus-app/ccfocus-app/ManualPairingsStore.swift`
- Create: `ccfocus-app/ccfocus-appTests/ManualPairingsStoreTests.swift`
- Create: `ccfocus-app/ccfocus-app/ManualPairView.swift`
- Modify: `ccfocus-app/ccfocus-app/MenuBarView.swift`
- Modify: `ccfocus-app/ccfocus-app/AppState.swift` (pairings 読み書きと起動時ロード)
- Modify: `ccfocus-app/ccfocus-app/LivenessChecker.swift` (削除済みterminalのpairingを自動クリーンアップ)
...
struct ManualPairingsStore {
    ...
    static func defaultURL() -> URL {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return support.appendingPathComponent("ccfocus/manual_pairings.json")
    }
}
```
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/ManualPairingsStoreTests test`
Expected: 3 passed
...
候補取得 `candidates` は `enumerate_terminals` 相当のロジックをswift側にも用意する (AppleScriptで全terminal列挙 → 配列化)。`ccfocus-app/ccfocus-app/GhosttyFocus.swift` に `GhosttyFocus.listTerminals() -> [(id:, cwd:, name:)]` を追加。
...
- ccfocus.appを `Quit` して再起動 → 先ほど紐付けたpaneがメニュー行のクリックで再びfocus飛ぶ (永続化が効いている)
...
Files:
- Create: `ccfocus-app/ccfocus-app/LogRotator.swift`
- Create: `ccfocus-app/ccfocus-appTests/LogRotatorTests.swift`
...
Run: `xcodebuild -project ccfocus-app/ccfocus-app.xcodeproj -scheme ccfocus-app -only-testing:ccfocus-appTests/LogRotatorTests test`
Expected: 1 passed
...
Expected: `ccfocus-app.app ccfocus-logger`

Run: `open dist/ccfocus-app.app`
...
Files:
- Create: `ccfocus-logger/src/commands/install.rs`

- [x] Step 1: `ccfocus-logger install` で `~/.claude/settings.json` に5hookをマージ
...
仕様カバレッジ (ccfocus-design.md との照合):
