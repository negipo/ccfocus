...
- 参照: tmp/doc/ccsplit-design.md (全設計と不変条件)
...
Files:
- Create: `tmp/verify/verify_hook.sh`
- Create: `tmp/verify/verify_ghostty.applescript`
- Create: `tmp/verify/results.md`

- [ ] Step 1: `tmp/verify/` ディレクトリを作成

Run: `mkdir -p tmp/verify`
Expected: エラーなし

- [ ] Step 2: `tmp/verify/verify_hook.sh` を作成
...
Run: `chmod +x tmp/verify/verify_hook.sh`
Expected: 作成成功

- [ ] Step 3: `tmp/verify/verify_ghostty.applescript` を作成
...
Run: `bash -n tmp/verify/verify_hook.sh && echo "bash syntax ok"`
Expected: `bash syntax ok`

- [ ] Step 5: 検証足場コミット

```bash
git add tmp/verify/verify_hook.sh tmp/verify/verify_ghostty.applescript
git commit
```
...
- [ ] Step 1: 検証用のhook設定を `~/.claude/settings.json` に一時追加

追加内容 (SessionStart array の先頭に):
```json
{"hooks": [{"type": "command", "command": "/Users/negipo/src/github.com/negipo/ccsplit/tmp/verify/verify_hook.sh"}]}
```

既存のSessionStart hookがある場合は末尾に追加する。ユーザに対し `update-config` スキル相当の編集を行う前に、現在のsettings.jsonの内容を提示して確認する。

- [ ] Step 2: Ghosttyで新しいpaneを開き、そのpane内で `cd /Users/negipo/src/github.com/negipo/ccsplit && claude` を実行
...
- [ ] Step 4: 結果を `tmp/verify/results.md` にまとめる
...
- [ ] Step 5: 検証A用の一時hook設定を `~/.claude/settings.json` から削除
...
- [ ] Step 1: Task 0.2で取得した hook txtの `--- osascript Ghostty terminal dump ---` セクションを確認

確認項目:
- `claude` を実行したpaneに相当する行の `name=` が `Claude Code` になっているか
- そうでない場合、どの値になっているか (shell prompt, cwd basename等)

- [ ] Step 2: 追加検証 - リトライありで観測

`tmp/verify/verify_hook.sh` に以下を追加 (Task 0.1のスクリプトを編集):
...
- [ ] Step 5: 検証用hookを settings.json から完全削除
...
Files:
- Create: `tmp/verify/verify_focus.applescript`

- [ ] Step 1: `tmp/verify/verify_focus.applescript` を作成
...
- [ ] Step 2: 実験対象となるpaneのIDを事前取得

Run: `osascript tmp/verify/verify_ghostty.applescript`
Expected: `id=<TID> | name=... | wd=...` 形式で複数paneが列挙される。テスト対象の `TID` をメモ。

- [ ] Step 3: 別のpaneをクリックでアクティブにしてから `verify_focus.applescript` を実行

ccsplit.appが実行するコンテキスト (バックグラウンド) に近づけるため、`caffeinate -s osascript ... &` のようにバックグラウンド実行で試すバリエーションも試す。

Run: `osascript tmp/verify/verify_focus.applescript <TID>`
...
- [ ] Step 4: Swift側で `NSAppleScript` 経由で同じ動作が再現するか試すミニマル Swift スクリプト

`tmp/verify/verify_focus_swift.swift`:
```swift
import Foundation
...
Run: `swift tmp/verify/verify_focus_swift.swift <TID>`
Expected: `focused` あるいは ok、指定paneに実際にfocusが飛ぶ

- [ ] Step 5: 結果を `tmp/verify/results.md` の検証Cセクションに追記
...
```bash
git add tmp/verify/results.md tmp/verify/verify_focus.applescript tmp/verify/verify_focus_swift.swift
git commit
```
...
- [ ] Step 3: `ccsplit-logger/Cargo.toml` を作成
...
- [ ] Step 4: `ccsplit-logger/src/main.rs` を作成
...
- [ ] Step 5: `ccsplit-logger/src/cli.rs` を作成
...
- [ ] Step 2: 生成された `ccsplit-app/ccsplit-app/ccsplit_appApp.swift` (Xcode自動命名) を `ccsplitApp.swift` にリネームし、中身を差し替える
...
- [ ] Step 2: `ccsplit-logger/src/detach.rs` を作成
...
- [ ] Step 1: テスト `ccsplit-logger/tests/event_serde.rs` を作成
...
- [ ] Step 3: `ccsplit-logger/src/event.rs` を作成
...
- [ ] Step 3: `ccsplit-logger/src/log_path.rs` を作成
...
- [ ] Step 4: `ccsplit-logger/src/lib.rs` に `pub mod log_path;` 追加
...
- [ ] Step 3: `ccsplit-logger/src/log_writer.rs` を作成
...
- [ ] Step 4: `ccsplit-logger/src/lib.rs` に `pub mod log_writer;` 追加
...
- [ ] Step 1: テスト (ドキュメントテストでOK、外形ルールのみ)
...
- [ ] Step 2: `ccsplit-logger/src/timestamp.rs` 作成
...
- [ ] Step 2: `ccsplit-logger/src/git.rs` 作成
...
- [ ] Step 2: `ccsplit-logger/src/claude_proc.rs` 実装
...
- [ ] Step 3: 親側 (is_detached_child() == false) での claude_pid 抽出を `detach.rs` と連携
...
- [ ] Step 1: AppleScriptテキスト定数と parser をテストファースト
...
- [ ] Step 2: `ccsplit-logger/src/ghostty.rs` 作成
...
pub fn pick_match(terms: &[Term], cwd: &str) -> MatchResult {
    let cands: Vec<&Term> = terms.iter()
        .filter(|t| t.wd == cwd && t.name == "Claude Code")
        .collect();
    match cands.len() {
        0 => MatchResult::None,
...
- [ ] Step 1: stdin payload parse のテスト
...
- [ ] Step 3: `cli.rs` から `Command::SessionStart => commands::session_start::run()` に結線
...
- [ ] Step 2: Stop / PreToolUse / UserPromptSubmit も同様にpayloadだけ拾ってjsonl追記
...
- [ ] Step 3: cli.rsに4コマンドを結線

- [ ] Step 4: ユニットテストでそれぞれpayload parseを確認 (各コマンドの tests モジュールに追加)
...
- [ ] Step 1: 実装 (ロジック薄いのでテストスキップ、手動確認)
...
- [ ] Step 1: テスト - オフセット管理と増分読み出しの純ロジックだけテスト
...
- [ ] Claude Code本家へのSessionEnd hook提案内容を `tmp/doc/upstream-claude-code-session-end.md` にまとめる
- [ ] Ghostty本家へのsurface_id提案内容を `tmp/doc/upstream-ghostty-surface-id.md` にまとめる
