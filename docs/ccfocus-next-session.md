# ccfocus 次セッション指示 (計画承認 → 実装開始)

## 現在位置

- レポジトリ: /Users/negipo/src/github.com/negipo/ccfocus
- ブランチ: po/ccfocus-impl-plan (作業中、main ブランチから切った)
- 計画: tmp/doc/ccfocus-impl-plan.md (まだコミットしていない)
- 設計: tmp/doc/ccfocus-design.md (前セッションで確定)
- 関連issue: negipo/ccfocus#1, #2, #3

## 前セッションで何をしたか

1. 設計ドキュメント tmp/doc/ccfocus-design.md を全文読込
2. superpowers:writing-plans スキルで tmp/doc/ccfocus-impl-plan.md を作成
...
4. 修正内容:
  ...
  - Task 2.8 を env経由 (CCFOCUS_CLAUDE_PID/START/COMM) でclaude情報継承する形に変更

## 次セッションの最初にやること

1. ユーザに計画をレビューしてもらう
  - 気になる箇所 (とくに Task 0.4 検証C、Task 1.2 Xcode project 手動操作、Task 2.0 self-detach) に対するユーザの見解を聞く
  - difit-review スキルで差分レビューに回すのも選択肢 (po/ccfocus-impl-plan の差分は tmp/doc/ccfocus-impl-plan.md 1ファイルのみ)
2. ユーザ承認が取れたら、計画ファイルをコミット (git-committing スキル)
  - メッセージ例: `docs: draft ccfocus implementation plan with phased verification and self-detach logger`
...
- Xcode project 作成 (Task 1.2) はエージェント不可。ユーザに手動依頼が必要
  - GUI 操作: Xcode で macOS App テンプレート、Product Name ccfocus-app、Bundle Identifier com.negipo.ccfocus-app、SwiftUI、Include Tests
